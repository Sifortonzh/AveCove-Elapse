...
from elapse_forge.ocr import parse_mineru_table_html
...
PROFILES = {
    "infectious-renwei": BuildProfile(
        ...
        answer_source="《传染病学学习指导与习题集》第3版（《传染病学》第9版配套）参考答案；MinerU Markdown 转换，使用时请结合原书复核。",
...
def main():
    ...
    parsed = reconcile(parse_document(build_document(args.inputs, profile)))
    selected = [
        question for question in parsed.questions if suitable(question, profile)
    ]
    unique = {}
