...
- 兼容医学题库常见的题干末尾答案、分章答案表、章节内题号重排，并分别识别单选、多选与判断题。
- 填空、名词解释、简答和病例论述会被主动跳过，不会硬塞进选择题界面；暂无答案的客观题会保留为测试模式。
...
- 根据答案数据自动识别单选题与多选题。
- 西医综合 306 按年份识别卷型：现代卷保留 A/B/X 型题并按 165 题、300 分计分；旧卷可保留 C 型“两陈述四判定”，不会误当多选。
- 内置“西综 306 标准化工作台”：可同时选择原卷与答案/解析卷，按页面和题号分段交给 AI 整理，检查 A/B/C/X 数量、缺失题号与答案覆盖率，并导出标准 JSON。
- 可选择“仅做单选”或“单选＋多选”。
