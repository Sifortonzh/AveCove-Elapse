...
> [!IMPORTANT]
> **当前版本：v2.3.3。** 章节目录会自动展开当前题所在章节并定位到当前题；普通红豆 JSON 导入时会把“单”规范为 A1 型、把“多”规范为 X 型。
...
| **AI 辅学** | 个人或全站 OpenAI 兼容接口，用于解析追问与写入笔记；普通导入不自动调用 AI | 可选 |
...
| **Word / PDF 工作台** | 本地提取或 OCR、题数与答案数统计、前三题轻量预览、确认后保存；失败时可导出源 JSON 给外部 AI 整理 | 稳定 |
...
普通导入优先使用红豆 JSON，这是最快的路径。Word / PDF 现在进入轻量识别工作台：Word 提取文字，PDF 只使用文字提取和 OCR，显示题数、答案数及前三题，确认后才保存。若识别失败，可下载提取文字 JSON，并复制内置标准化提示词交给自选 AI；取得红豆题库 JSON 后再导入。上传文件不会自动触发 AI 识别。藏经阁上传的是独立 JSON 副本，删除“我的题库”中的文件不会删除已发布副本。
...
| **当前重点** | 章节型医学练习册、跨页题目和全书末尾答案表的确定性 Word / JSON 导入 |
...
- **v2.3.3** — 章节目录自动展开并定位当前题；普通 JSON 的“单/多”题型自动规范为 A1/X。
