...
> 当前版本线：`0.4.x` 预览版。项目只内置 8 道演示题，不包含完整儿科学课程题库或其他受版权保护的商业题库。
...
- 自动判断学习阶段，并拆分完形、阅读、段落匹配、翻译、听力资源和写作章节。
...
AI 兜底不会向 AI 厂商发送原始文档，只发送已经提取的文字，并且必须先获得用户确认。使用前应确认文件中不含患者资料、个人隐私或其他敏感信息。AI 识别仍可能出错，导入后应抽查题目与答案。
